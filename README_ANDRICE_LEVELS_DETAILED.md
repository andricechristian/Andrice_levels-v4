...
$$u_i^{\%}=100\frac{H_i-O_i}{O_i},\qquad d_i^{\%}=100\frac{O_i-L_i}{O_i}.$$
...
$$TR_i=\max\left(H_i-L_i,\;|H_i-C_{i-1}^{complete}|,\;|L_i-C_{i-1}^{complete}|\right).$$

Without a previous close, the first TR is $H_i-L_i$. The initial ending ATR is seeded after 14 accepted sessions:

$$ATR^{end}_{14}=\frac{1}{14}\sum_{i=1}^{14}TR_i.$$

Subsequent values use Wilder smoothing:

$$ATR^{end}_i=\frac{13ATR^{end}_{i-1}+TR_i}{14},\qquad ATR^{previous}_i=ATR^{end}_{i-1}.$$
...
$$u_i^{ATR}=\frac{H_i-O_i}{ATR^{previous}_i},\qquad d_i^{ATR}=\frac{O_i-L_i}{ATR^{previous}_i}.$$
...
For any one side and normalization, denote its $n$ valid observations by $x_1,\ldots,x_n$:
...
For bootstrap estimates $\hat\theta_1^*,\ldots,\hat\theta_B^*$:

$$SE_{Block}(\hat\theta)=\sqrt{\frac{\sum_{b=1}^{B}(\hat\theta_b^*-\bar\theta^*)^2}{B-1}}.$$
...
For one excursion distribution, $Q_q$ denotes its sample quantile at fraction $q$. `P75` and `Q75` refer to the same percentile threshold, but the code uses `P75` in descriptive statistics and `Q75` as an envelope name.

The current NumPy calculation uses linear interpolation. With sorted values $x_{(1)},\ldots,x_{(n)}$, define $h=(n-1)q$, $j=\lfloor h\rfloor$, and $g=h-j$. Using zero-based sorted indices, $Q_q=(1-g)x_j+gx_{j+1}$, with the endpoint handled directly. This explains fractional results even when input prices are tick-aligned.
...
Let $O_*$ be the actual open of the session being forecast. Let $A_*$ be the known prior RTH ATR. Define a positive scale:

$$k=\begin{cases}1&\text{points model},\\O_*/100&\text{percentage model},\\A_*&\text{ATR model}.\end{cases}$$

For any chosen historical upside statistic $x_u$ and downside statistic $x_d$:

$$Upper=O_*+kx_u,\qquad Lower=O_*-kx_d.$$
...
$$HOD\ zone=[O_*+ka,O_*+kb],$$
$$LOD\ zone=[O_*-kb,O_*-ka].$$
...
The current calculation treats $O_*$ and $A_*$ as supplied constants. It does not propagate uncertainty in the opening price or ATR model itself. If a downside confidence interval is transformed manually, reverse its endpoints: $[a,b]$ in distances becomes $[O_*-kb,O_*-ka]$ in prices.
...
$$U_q=O_*+kQ_q^{up},\qquad L_q=O_*-kQ_q^{down}.$$

Envelope width is $U_q-L_q=k(Q_q^{up}+Q_q^{down})$. It is the **total width**, not the distance on each side of the open. A 160-point envelope can have an upside distance of 90 and a downside distance of 70.

### Joint probability is not a marginal percentile

RTH containment is the joint event:

$$H_{RTH}\le U_q\quad\text{and}\quad L_{RTH}\ge L_q.$$
...
- Mean time, with $s_t/\sqrt n$ IID uncertainty and bootstrap mean uncertainty, in minutes.
...
$$\hat p_j=\frac{\text{valid extreme labels in window }j}{\text{all valid labels for that side}},\qquad SE_{IID}(\hat p_j)=\sqrt{\frac{\hat p_j(1-\hat p_j)}{n}}.$$
...
Let $N_L$ and $N_H$ be the known LOD-first and HOD-first counts:

$$Ratio_{LOD/HOD}=\frac{N_L}{N_H},\qquad Share_{LOD-first}=\frac{N_L}{N_L+N_H}.$$
...
Let $U,L$ be the frozen outer bounds and $H_t,L_t,C_t$ the realized high, low, and close. The implementation uses a numerical tolerance of $10^{-9}$ points for comparisons; it is not a tradable tick buffer.

| Metric | Event / definition |
| --- | --- |
| `Contained_RTH` | $H_t\le U$ and $L_t\ge L$. The entire observed RTH range remained inside. |
| `HOD_Within_Upper` | $H_t\le U$. Upper marginal respect. |
| `LOD_Within_Lower` | $L_t\ge L$. Lower marginal respect. |
| `Close_Inside` | $L\le C_t\le U$, even if earlier prices breached. |
| `Upper_Reached` | $H_t\ge U$. Threshold reached or exceeded, not proof of a fill. |
| `Lower_Reached` | $L_t\le L$. Same interpretation for downside. |
| `Upper_Breached` | $H_t>U$. |
| `Lower_Breached` | $L_t<L$. |
...
$$Width=U-L,\qquad Width_{ATR}=\frac{U-L}{A_t}.$$

Overshoots are:

$$Overshoot_{up}=\max(H_t-U,0),\qquad Overshoot_{down}=\max(L-L_t,0).$$
...
The backtester uses upside error $e_u=H_t-U$ and downside distance error $e_d=L-L_t$:

$$Pinball\_Loss\_ATR=\frac{\rho_q(e_u)+\rho_q(e_d)}{A_t}.$$
...
For binary outcomes $I_t$ across $N$ eligible forecasts:

$$\hat p=\frac{\sum I_t}{N},\qquad SE_{IID}=\sqrt{\frac{\hat p(1-\hat p)}{N}}.$$
...
| Sample SD | $\sqrt{\sum(x_i-\bar{x})^2/(n-1)}$ |
...
| Upside / downside price | $O_*+kx$ / $O_*-kx$ |
| Projected boundary SE | $kSE_x$, conditional on the supplied scale |
| Timing offset | $60h+m+s/60-570$ minutes |
| LOD-first share | $N_L/(N_L+N_H)$ |
| LOD/HOD order ratio | $N_L/N_H$ when $N_H>0$ |
| Event rate | $Hits/N$ |
| Independent event-rate SE | $\sqrt{\hat p(1-\hat p)/N}$ |
| Entire RTH contained | $H_t\le U$ and $L_t\ge L$ |
| Total overshoot | $\max(H_t-U,0)+\max(L-L_t,0)$ |
