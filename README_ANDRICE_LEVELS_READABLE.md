...
**Readable-formula edition — 13 September 2026.** All equations below use ordinary text and code formatting. No LaTeX, MathJax, or other equation renderer is required. The project explanations and calculations are retained; formula notation has been rewritten for readability.

### Reading the formula notation

- `*` means multiplication; `/` means division; `^2` means squared. These are explanatory formulas, not executable Python (`**2` would be Python exponentiation).
- `sqrt(x)` is the square root; `abs(x)` is absolute value; `sum(...)` adds the listed terms; `mean(...)` takes their arithmetic average.
- `min(...)` and `max(...)` choose the smallest and largest values; `floor(x)` rounds down to an integer.
- `x[i]` means the observation for session `i`; `mean_x` is the sample mean.
- `Q(q)` is the historical quantile at probability `q`; `Q_up(q)` and `Q_down(q)` keep the two sides separate.
- `Open_today` means the known RTH open of the session being forecast, including when forecasting a historical date. `ATR_before_today` means its known prior RTH ATR.
- `<=` and `>=` mean less than or equal to and greater than or equal to; `+/-` means plus or minus.
- Square brackets around two bounds, such as `[a, b]`, describe an interval. Bracketed indices such as `x[i]` instead identify observations.
- Mathematical variable names are explanatory. Exact CSV field names remain in the column dictionaries.
...
```text
S(D) = (O_RTH(D), H_RTH(D), L_RTH(D))
```


The historical data range was written as `DATA_RNG = [1, n]`. Here `D` is the position of a session in the dataset. It is not the day of the month, and it does not imply that every intervening calendar date has a row. The later addition of `Date` gives each observation an unambiguous real date.

Two distances were defined separately:


```text
PUV(D) = H_RTH(D) - O_RTH(D)
```


```text
PDV(D) = O_RTH(D) - L_RTH(D)
```
...
| `PUV_ATR` | `(H - O) / ATR14_Previous` | Dimensionless ATR multiple; blank when prior ATR is unavailable or not positive. |
| `PDV_ATR` | `(O - L) / ATR14_Previous` | Same convention for the downside. |
...
```text
u_percent[i] = 100 * (H[i] - O[i]) / O[i]
d_percent[i] = 100 * (O[i] - L[i]) / O[i]
```
...
Let `Previous_Complete_RTH_Close` be the previous eligible completed RTH close in the same ATR sequence:


```text
TR[i] = max(
    H[i] - L[i],
    abs(H[i] - Previous_Complete_RTH_Close),
    abs(L[i] - Previous_Complete_RTH_Close)
)
```


Without a previous close, the first TR is `H[i] - L[i]`. The initial ending ATR is seeded after 14 accepted sessions:


```text
ATR_End[14] = (TR[1] + TR[2] + ... + TR[14]) / 14
```
...
```text
ATR_End[i] = (13 * ATR_End[i-1] + TR[i]) / 14
ATR_Previous[i] = ATR_End[i-1]
```
...
```text
u_ATR[i] = (H[i] - O[i]) / ATR_Previous[i]
d_ATR[i] = (O[i] - L[i]) / ATR_Previous[i]
```
...
For any one side and normalization, denote its `n` valid observations by `x[1], ..., x[n]`:


```text
mean_x = (x[1] + x[2] + ... + x[n]) / n
```
...
```text
AVG_PUV = 298 / 7 = 42.571429 points
AVG_PDV = 301 / 7 = 43.000000 points
```
...
```text
s = sqrt(sum((x[i] - mean_x)^2, for i = 1 to n) / (n - 1))
```
...
```text
standard_uncertainty_of_mean = SE_IID = s / sqrt(n)
```
...
For bootstrap estimates `estimate_bootstrap[1], ..., estimate_bootstrap[B]`:


```text
mean_bootstrap = mean(estimate_bootstrap[1], ..., estimate_bootstrap[B])
SE_Block = sqrt(
    sum((estimate_bootstrap[b] - mean_bootstrap)^2, for b = 1 to B)
    / (B - 1)
)
```
...
```text
effective_block_length = min(requested_block_length, max(1, floor(n / 3)))
```
...
For one excursion distribution, `Q(q)` denotes its sample quantile at fraction `q`. `P75` and `Q75` refer to the same percentile threshold, but the code uses `P75` in descriptive statistics and `Q75` as an envelope name.

The current NumPy calculation uses linear interpolation. With sorted values `x_sorted[1], ..., x_sorted[n]`, define `h = (n - 1) * q`, `j = floor(h)`, and `g = h - j`. Using zero-based sorted indices, `Q(q) = (1 - g) * x_sorted[j] + g * x_sorted[j+1]`, with the endpoint handled directly. This explains fractional results even when input prices are tick-aligned.
...
| --- | --- | --- | --- |
| `MeanSE` | `[max(0, mean_x - SE_IID), mean_x + SE_IID]` | Mean | Precision-oriented region around the estimated average excursion. |
| `P25_P75` | `[Q(0.25), Q(0.75)]` | Median, `Q(0.50)` | Middle approximately 50% of the historical excursion distribution. |
| `P10_P90` | `[Q(0.10), Q(0.90)]` | Median, `Q(0.50)` | Middle approximately 80% of the historical excursion distribution. |
...
Let `Open_today` be the actual open of the session being forecast. Let `ATR_before_today` be the known prior RTH ATR. Define a positive scale:


```text
Points model:      k = 1
Percentage model:  k = Open_today / 100
ATR model:         k = ATR_before_today
```


For any chosen historical upside statistic `up_statistic` and downside statistic `down_statistic`:


```text
Upper = Open_today + k * up_statistic
Lower = Open_today - k * down_statistic
```
...
If a distance zone is `[a, b]`, with `a <= b`:


```text
HOD zone = [Open_today + k * a, Open_today + k * b]
```


```text
LOD zone = [Open_today - k * b, Open_today - k * a]
```
...
The current calculation treats `Open_today` and `ATR_before_today` as supplied constants. It does not propagate uncertainty in the opening price or ATR model itself. If a downside confidence interval is transformed manually, reverse its endpoints: `[a, b]` in distances becomes `[Open_today - k * b, Open_today - k * a]` in prices.
...
| `MeanPlusSE` | Separate `mean_x + SE_IID` distances | Uses the outward edges of the two `MeanSE` zones; the lower price is open minus the larger downside distance. |
...
```text
Upper_q = Open_today + k * Q_up(q)
Lower_q = Open_today - k * Q_down(q)
```


Envelope width is `Upper_q - Lower_q = k * (Q_up(q) + Q_down(q))`. It is the **total width**, not the distance on each side of the open. A 160-point envelope can have an upside distance of 90 and a downside distance of 70.
...
```text
H_RTH <= Upper_q AND L_RTH >= Lower_q
```
...
```text
max(0, 2 * q - 1) <= probability(both contained) <= q
```
...
- Mean time, with `s_time / sqrt(n)` IID uncertainty and bootstrap mean uncertainty, in minutes.
...
```text
share[j] = valid_extreme_labels_in_window[j] / all_valid_labels_for_that_side
SE_IID(share[j]) = sqrt(share[j] * (1 - share[j]) / n)
```
...
- `LOD_FIRST` if `time_LOD < time_HOD`.
- `HOD_FIRST` if `time_HOD < time_LOD`.
- `SAME_BAR` when both first occurrences have the same bar label.
- Missing timing pairs cannot establish order.

Let `N_LOD_first` and `N_HOD_first` be the known LOD-first and HOD-first counts:


```text
LOD_to_HOD_Ratio = N_LOD_first / N_HOD_first
LOD_First_Share = N_LOD_first / (N_LOD_first + N_HOD_first)
```
...
Let `Upper` and `Lower` be the frozen outer bounds and `High[t]`, `Low[t]`, and `Close[t]` the realized high, low, and close. The implementation uses a numerical tolerance of `0.000000001` points for comparisons; it is not a tradable tick buffer.

| Metric | Event / definition |
| --- | --- |
| `Contained_RTH` | `High[t] <= Upper` and `Low[t] >= Lower`. The entire observed RTH range remained inside. |
| `HOD_Within_Upper` | `High[t] <= Upper`. Upper marginal respect. |
| `LOD_Within_Lower` | `Low[t] >= Lower`. Lower marginal respect. |
| `Close_Inside` | `Lower <= Close[t] <= Upper`, even if earlier prices breached. |
| `Upper_Reached` | `High[t] >= Upper`. Threshold reached or exceeded, not proof of a fill. |
| `Lower_Reached` | `Low[t] <= Lower`. Same interpretation for downside. |
| `Upper_Breached` | `High[t] > Upper`. |
| `Lower_Breached` | `Low[t] < Lower`. |
...
```text
Width = Upper - Lower
Width_ATR = (Upper - Lower) / ATR_Previous[t]
```
...
```text
Overshoot_up = max(High[t] - Upper, 0)
Overshoot_down = max(Lower - Low[t], 0)
```
...
The backtester uses upside error `error_up = High[t] - Upper` and downside distance error `error_down = Lower - Low[t]`:


```text
Pinball_Loss_ATR = (pinball(q, error_up) + pinball(q, error_down))
                   / ATR_Previous[t]
```
...
For binary outcomes `hit[t]` across `N` eligible forecasts:


```text
p_hat = sum(hit[t], across the N evaluated forecasts) / N
SE_IID = sqrt(p_hat * (1 - p_hat) / N)
```
...
half_width = z * sqrt(p_hat * (1 - p_hat) / N + z^2 / (4 * N^2))
             / denominator
Wilson95_Low = center - half_width
Wilson95_High = center + half_width
```
...
| PUV / PDV ATR | `(H - O) / ATR_Previous` / `(O - L) / ATR_Previous` |
| RTH range | `H - L = PUV + PDV` |
| True range with prior close | `max(H - L, abs(H - Previous_Close), abs(L - Previous_Close))` |
| ATR14 seed | Mean of first 14 TRs in the accepted sequence |
| Wilder continuation | `(13 * ATR_Previous + TR) / 14` |
| Sample mean | `sum(x[i], for i = 1 to n) / n` |
| Sample SD | `sqrt(sum((x[i] - mean_x)^2, for i = 1 to n) / (n - 1))` |
...
| MeanSE distance bounds | `max(0, mean_x - s / sqrt(n))` and `mean_x + s / sqrt(n)` |
| Central distance zones | `[Q(0.25), Q(0.75)]` and `[Q(0.10), Q(0.90)]` |
| Upside / downside price | `Open_today + k * x` / `Open_today - k * x` |
| Projected boundary SE | `k * SE_x`, conditional on the supplied scale |
| Timing offset | `60 * hour + minute + second / 60 - 570` minutes |
| LOD-first share | `N_LOD_first / (N_LOD_first + N_HOD_first)` |
| LOD/HOD order ratio | `N_LOD_first / N_HOD_first` when `N_HOD_first > 0` |
| Event rate | `Hits / N` |
| Independent event-rate SE | `sqrt(p_hat * (1 - p_hat) / N)` |
| Entire RTH contained | `High[t] <= Upper` and `Low[t] >= Lower` |
| Total overshoot | `max(High[t] - Upper, 0) + max(Lower - Low[t], 0)` |
...
[the complete Andrice Levels guide](README_ANDRICE_LEVELS_READABLE.md).
```
